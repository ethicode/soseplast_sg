...
class Product {
    ...
    public function addProduct($username, $email, $password) {
        // Prépare la requête SQL pour insérer un nouvel utilisateur.
        // Utilise des marqueurs de position "?" pour les valeurs à insérer.
        $stmt = $this->db->prepare("INSERT INTO product (name, description, quantity) VALUES (?, ?, ?)");
    ...
    public function updateProduct($id, $username, $email) {
        $stmt = $this->db->prepare("UPDATE product SET username = ?, email = ? WHERE id = ?");
        $stmt->bind_param("ssi", $username, $email, $id);
        $stmt->execute();
        $stmt->close();
    }

    public function deleteProductr($id) {
