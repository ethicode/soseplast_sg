...
class Request
{
    ...
    public function getDetailRequest($uuid)
    {
        $stmt = $this->db->prepare("SELECT * FROM request WHERE uuid = ?");
        $stmt->bind_param('i', $uuid); // "i" signifie que le paramètre est un entier.
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    {
        $stmt = $this->db->prepare("SELECT request.id, request.title, request.description, request.created_at, request.status, request.is_validated, user.name FROM request LEFT JOIN user on request.user_id=user.id ORDER BY request.id DESC  LIMIT ?,?;");
    ...
    public function saveRequest($uuid, $text_1, $text_2, $text_3, $user_id, $fichier)
    {

        $stmt = $this->db->prepare("INSERT INTO request (uuid, text_1, text_2, text_3, user_id, fichier) VALUES (?, ?, ?, ?, ?, ?)");

        if ($stmt === false) {
            die("MySQL Error: " . $this->db->error);
        }
        // Lie les variables aux marqueurs de position dans la requête préparée.
        $stmt->bind_param("ssssis", $uuid, $text_1, $text_2, $text_3, $user_id, $fichier);
